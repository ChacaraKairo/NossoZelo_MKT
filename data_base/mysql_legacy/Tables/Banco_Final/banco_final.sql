...
CREATE TABLE `agenda_recorrente` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `prestador_id` VARCHAR(20) NOT NULL,
    `dia_semana` ENUM('domingo', 'segunda', 'ter├ºa', 'quarta', 'quinta', 'sexta', 's├íbado') NULL,
    `hora_inicio` TIME(0) NOT NULL,
    `hora_fim` TIME(0) NOT NULL,
...
CREATE TABLE `avaliacoes` (
    ...
    UNIQUE INDEX `avaliacoes_contratacao_id_key`(`contratacao_id`),
...
CREATE TABLE `contratacoes` (
    ...
    `status` ENUM('pendente', 'confirmado', 'concluido', 'paga', 'cancelado', 'manual') NULL DEFAULT 'pendente',
    `observacoes` TEXT NULL,
