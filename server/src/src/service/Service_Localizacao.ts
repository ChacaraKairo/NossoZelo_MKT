...
export class GeolocalizacaoService {
  ...
  public static async buscarPrestadores(params: {
    ...
    try {
      ...
      // Filtro de Tipo
      if (tipoFiltro) {        conditions.push(Prisma.sql`u.tipo = ${tipoPrestadorSql(tipoFiltro)}`);
      } else {        conditions.push(
          Prisma.sql`u.tipo IN (${tiposPrestadorSql()})`,
        );
      }
  ...
  public static async buscar20UsuariosMaisProximosPorTipo(
    ...
    tipo: string,
      ...
      >`
        ...
        FROM usuarios u
        INNER JOIN localizacoes l ON u.id = l.usuario_id
        WHERE u.id != ${usuarioId} AND u.tipo = ${tipoPrestadorSql(tipo)}
        AND u.email_confirmado = true
        AND u.status_cadastro = 'ativo'::usuarios_status_cadastro
        AND EXISTS (
          SELECT 1
