...
console.log(
  '[LOG-FLUXO] Carregando variáveis de ambiente (dotenv) para o EmailService.',
);
dotenv.config();
...
export class EmailService {
  ...
  constructor() {
    console.log(
      `[LOG-FLUXO] Inicializando construtor do EmailService. Host: ${process.env.EMAIL_HOST}, Porta: ${process.env.EMAIL_PORT}`,
    );
    ...
    console.log(
      '[LOG-FLUXO] Configuração do transporte SMTP do Nodemailer concluída com sucesso.',
    );
  }
  ...
  async send(
    ...
    console.log(
      `[LOG-FLUXO] Iniciando execução do método send. Destinatário: ${to}, Assunto: ${subject}`,
    );

    try {
      console.log(
        `[LOG-FLUXO] Preparando metadados de envio. Remetente configurado: ${process.env.EMAIL_USER}`,
      );

      // Execução da chamada assíncrona ao servidor de e-mail
      console.log(
        `[LOG-FLUXO] Invocando transporte assíncrono para tentativa de disparo para: ${to}`,
      );
      ...
      console.log(
        `[LOG-FLUXO] 📧 E-mail despachado com sucesso para ${to}. Identificador único (MessageID): ${info.messageId}`,
      );
      ...
      console.error(
        `[ERRO-FLUXO] Falha crítica no disparo de e-mail para ${to}: Ocorreu uma interrupção na comunicação SMTP. Detalhes: ${
          error.message || error
        }`,
      );
