...
function ambienteAsaas(): AmbienteAsaas {
  ...
  if (['production', 'producao', 'prod'].includes(ambiente)) {
    return 'production';
  }

  if (['sandbox', 'homologacao', 'homologation', 'test'].includes(ambiente)) {
    return 'sandbox';
  }

  throw new Error(
    'ASAAS_ENVIRONMENT invalido. Use sandbox ou production.',
  );
}

function validarCompatibilidadeBaseUrl(
