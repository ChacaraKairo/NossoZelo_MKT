...
function authMiddleware(req, res, next) {
    console.log(`[LOG-FLUXO] authMiddleware: Iniciando verificação de autorização para a rota: ${req.originalUrl}`);
    const authHeader = req.headers.authorization;
    // Ramificação condicional: Verificação de presença do Header
    if (!authHeader) {
        console.warn(`[LOG-FLUXO] Bloqueio de Acesso: Cabeçalho 'authorization' não encontrado na requisição vinda de ${req.ip}.`);
        return res
            .status(401)
            .json({ error: 'Token não fornecido' });
    }
    console.log('[LOG-FLUXO] Cabeçalho de autorização detectado. Extraindo token Bearer.');
    // Extração do token mantendo nomenclatura original
    const [, token] = authHeader.split(' ');
    try {
        console.log('[LOG-FLUXO] Solicitando validação criptográfica do token via jsonwebtoken (verify).');
         ...
         */
        req.user = decoded;
        console.log(`[LOG-FLUXO] Autorização bem-sucedida. Usuário autenticado vinculado à requisição. Prosseguindo fluxo.`);
        // Prossegue para o próximo middleware/controller
        return next();
    }
    catch (error) {
        console.error(`[ERRO-FLUXO] Falha na validação do token JWT: ${error.message ||
            'Token inválido ou assinatura corrompida'}. Rejeitando acesso.`);
        return res
            .status(401)
